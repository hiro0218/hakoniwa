...
$BACK_TO_TOP = "<a href=\"{$GAME_TOP}?\">{$init->tagBig_}トップへ戻る{$init->_tagBig}</a>、<A HREF=\"{$THIS_FILE}?\">{$init->tagBig_}同盟トップへ戻る{$init->_tagBig}</A>";
...
class HtmlAlly extends HTML {
	...
	function allyTop($hako, $data) {
		...
		print "<h1 class=\"title\">{$init->title}<br>同盟管理ツール</h1>\n";
		
		if($init->allyUse) {
			print <<<END
<input type="button" value="同盟の結成・変更・解散・加盟・脱退はこちらから" onClick="JavaScript:location.replace('{$GLOBALS['THIS_FILE']}?JoinA=1')">
<h2>各同盟の状況</h2>
END;
		}
		$this->allyInfo($hako);
...
END;
		...
		print "</table>\n";
		print "</div>\n";
		print "<b>※</b>同盟の名前をクリックすると「同盟の情報」欄へ、盟主島の名前だと「コメント変更」欄へ移動します。\n";
	}
	//--------------------------------------------------
	// 同盟の情報
	//--------------------------------------------------
...
END;
		if(!$ally['number']) {
			print "<TR><TH colspan=12>所属している島がありません！</TH></TR>";
		}
...
END;
	...
	function newAllyTop($hako, $data) {
		...
		$joinCost = $init->comCost[$init->comAlly] ? "{$init->comCost[$init->comAlly]}{$init->unitMoney}" : '無料';
		$joinStr = $init->allyJoinComUse ? '' : "加盟・脱退の際の費用は、{$init->tagMoney_}$joinCost{$init->_tagMoney}です。<BR>";
		$str3 = $adminMode ? "特殊パスワードは？（必須）<BR>
<INPUT TYPE=\"password\" NAME=\"OLDPASS\" VALUE=\"{$data['defaultPassword']}\" SIZE=32 MAXLENGTH=32 class=f><BR>同盟" : "<span class='attention'>(注意)</span><BR>
...
class AllySetted extends HtmlAlly {
	...
	function allyPactOK($name) {
		global $init;
		print "{$init->tagBig_}{$name}のコメントを変更しました。{$init->_tagBig}<br>{$GLOBALS['BACK_TO_TOP']}\n";
	}
	// 同盟データの再構成
	function allyDataUp() {
		global $init;
		print "{$init->tagBig_}同盟データを再構成しました。{$init->_tagBig}<br>{$GLOBALS['BACK_TO_TOP']}\n";
	}
}
//------------------------------------------------------------
//
...
class AllyError {
	...
	function newAllyAlready() {
		global $init;
		print "{$init->tagBig_}その同盟ならすでに結成されています。{$init->_tagBig}<br>{$GLOBALS['BACK_TO_TOP']}\n";
	}
	// すでにそのマークの同盟がある場合
	function markAllyAlready() {
		global $init;
		print "{$init->tagBig_}そのマークはすでに使用されています。{$init->_tagBig}<br>{$GLOBALS['BACK_TO_TOP']}\n";
	}
	// 別の同盟を結成している
	function leaderAlready() {
		global $init;
		print "{$init->tagBig_}盟主は、自分の同盟以外には加盟できません。{$init->_tagBig}<br>{$GLOBALS['BACK_TO_TOP']}\n";
	}
	// 別の同盟に加盟している
	function otherAlready() {
		global $init;
		print "{$init->tagBig_}ひとつの同盟にしか加盟できません。{$init->_tagBig}<br>{$GLOBALS['BACK_TO_TOP']}\n";
	}
	// 資金足りず
	function noMoney() {
		global $init;
		print "{$init->tagBig_}資金不足です(/_<。){$init->_tagBig}<br>{$GLOBALS['BACK_TO_TOP']}\n";
	}
	// IDチェックにひっかかる
	function wrongAlly() {
		global $init;
		print "{$init->tagBig_}あなたは盟主ではないと思う。{$init->_tagBig}<br>{$GLOBALS['BACK_TO_TOP']}\n";
	}
	// 新規で同盟がない場合
	function newAllyNoName() {
		global $init;
		print "{$init->tagBig_}同盟につける名前が必要です。{$init->_tagBig}<br>{$GLOBALS['BACK_TO_TOP']}\n";
	}
	// 管理者以外結成不可
	function newAllyForbbiden() {
		global $init;
		print "{$init->tagBig_}申し訳ありません、受付を中止しています。{$init->_tagBig}<br>{$GLOBALS['BACK_TO_TOP']}\n";
	}
}
...
class MakeAlly {
	...
	function makeAllyMain($hako, $data) {
		...
		if($n != '') {
			if($adminMode && ($allyID != '') && ($allyID < 200)) {
				...
				if(!$flag) {
					print "{$init->tagBig_}変更できません。{$init->_tagBig}<br>{$GLOBALS['BACK_TO_TOP']}\n";
					return;
				}
	...
	function deleteAllyMain($hako, $data) {
		...
		if($adminMode && (($allyMember[0] != '') || ($n == ''))){
			print "{$init->tagBig_}削除できません。{$init->_tagBig}<br>{$GLOBALS['BACK_TO_TOP']}\n";
			return;
		}
		foreach ($allyMember as $id) {
			$island = $hako->islands[$hako->idToNumber[$id]];
...
class Util {
	...
	function htmlEscape($s, $mode = 0) {
		...
		if ($mode) {
			...
			$s = str_replace("\n", "<br>", $s);
			$s = ereg_replace("(<br>){5,}", "<br>", $s); // 大量改行対策
		}
		return $s;
	}
